...
arcgis_server_server 'Install ArcGIS for Server' do
  ...
  if node['platform'] == 'windows'
    ...
    notifies :configure_autostart, 'arcgis_server_server[Configure arcgisserver service]', :immediately
  end
...
arcgis_server_datastore 'Install ArcGIS DataStore' do
  ...
  if node['platform'] == 'windows'
    ...
    notifies :configure_autostart, 'arcgis_server_datastore[Configure arcgisdatastore service]', :immediately
  end
  action [:system, :install]
end
...
arcgis_server_portal 'Install Portal for ArcGIS' do
  ...
  if node['platform'] == 'windows'
    ...
    notifies :configure_autostart, 'arcgis_server_portal[Configure arcgisportal service]', :immediately
  end
  action [:system, :install]
end
