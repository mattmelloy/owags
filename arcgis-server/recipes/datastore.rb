...
if node['platform'] == 'windows'
  arcgis_server_datastore 'Update ArcGIS Data Store service logon account' do
...
arcgis_server_datastore "Install System Requirements:#{recipe_name}" do
  action :system
  only_if { node['arcgis']['data_store']['install_system_requirements'] }
end

arcgis_server_datastore 'Install ArcGIS DataStore' do
...
arcgis_server_datastore 'Configure arcgisdatastore service' do
  install_dir node['arcgis']['data_store']['install_dir']
  only_if { node['arcgis']['data_store']['configure_autostart'] }
  action :configure_autostart
end

arcgis_server_datastore 'Start ArcGIS Data Store' do
  action :start
end

arcgis_server_datastore 'Configure ArcGIS DataStore' do
  install_dir node['arcgis']['data_store']['install_dir']
  data_dir node['arcgis']['data_store']['data_dir']
...
arcgis_server_datastore 'Change ArcGIS DataStore backup location' do
  install_dir node['arcgis']['data_store']['install_dir']
